...
from isaac_ros_manipulation_robot_utils.robot_controller_base import RobotControllerBase
...
class URDriverUtils(RobotControllerBase):
    """UR driver utilities."""

    def __init__(self, driver_config: UrRobotiqDriverConfig):
        super().__init__(driver_config)

    def get_visualization_nodes(
            self, core_config: CoreConfig, moveit_config: MoveItConfigsBuilder) -> Node:
        """
            ...
            if core_config.workflow_config.workflow_type in (
                ...
                return IncludeLaunchDescription(
                    ...
                    launch_arguments={
                        ...
                        'run_foxglove': (
                            'True' if core_config.enable_foxglove_visualization else 'False'),
                    }.items(),
                )
    ...
    def get_robot_state_publisher(self) -> Node:
        """
        Return the robot state publisher that publishes TF and robot description.

        Implements :meth:`RobotControllerBase.get_robot_state_publisher` for the
        UR + Robotiq family. Builds the ``robot_description`` from the URDF
        xacro via :func:`get_robot_description_contents_for_sim` (sim) or
        :func:`get_robot_description_contents_for_real_robot` (real), and
        remaps ``/joint_states`` to the sim-parsed topic in simulation.
        ...
        if driver_config.use_sim_time:
            robot_description_contents = get_robot_description_contents_for_sim(
                ...
                gripper_type=driver_config.gripper_type.value,
            ...
            robot_description_contents = get_robot_description_contents_for_real_robot(
                driver_config)
    ...
    def get_robot_control_nodes(self) -> List[Node]:
        """
        Return the ``ros2_control`` nodes and controller spawners for UR.

        Implements :meth:`RobotControllerBase.get_robot_control_nodes`. Dispatches
        between the simulated and real UR control stacks based on
        ``self.driver_config.use_sim_time``.

        Returns
        -------
            List[Node]: Nodes to launch (``ros2_control_node`` + spawners).

        """
        if self.driver_config.use_sim_time:
            return get_ros2_control_nodes_for_sim(self.driver_config)
        return get_ros2_control_nodes_for_real(self.driver_config)

    def get_moveit_group_node(self) -> Tuple[Node, Dict]:
        """
        Return the MoveIt ``move_group`` node and the matching config bundle.

        Implements :meth:`RobotControllerBase.get_moveit_group_node`. Builds a
        ``MoveItConfigsBuilder`` from the SRDF / kinematics / joint-limits /
        moveit-controllers paths on ``self.driver_config``, registers the
        cuMotion planning pipeline, and returns both the node and the builder
        so downstream consumers (e.g. RViz) can reuse the same robot
        description.

        Returns
        -------
            Tuple[Node, Dict]: The ``move_group`` node and the MoveIt config
        ...
        if driver_config.use_sim_time:
            ...
            robot_description_content = get_robot_description_contents_for_real_robot(
                driver_config)

        moveit_config = (
            MoveItConfigsBuilder(
                'ur_with_gripper',
                package_name='isaac_ros_manipulation_ur_robot_description')
...
def get_ros2_control_nodes_for_sim(driver_config: UrRobotiqDriverConfig) -> List[Node]:
    ...
    gripper_controller = Node(
        package='isaac_ros_manipulation_ur_isaac_sim_utils',
...
def get_ros2_control_nodes_for_real(driver_config: UrRobotiqDriverConfig) -> List[Node]:
    ...
    initial_joint_controllers = PathJoinSubstitution(
        [FindPackageShare('isaac_ros_manipulation_ur_robot_description'),
         'config', 'ros2_control_controllers.yaml']
    )

    robotiq_gripper_controllers = PathJoinSubstitution(
        [FindPackageShare('isaac_ros_manipulation_ur_robot_description'), 'config',
         f'{driver_config.gripper_type.value}_controllers.yaml']
    )
