...
class IsaacSimGripperActionServer(Node):
    ...
    def isaac_joint_states_callback(self, msg: JointState):
        ...
        If the action callback is happening, then we publish cached joint state. If not
        then, do nothing. This is to make sure gripper fingers dont start to twitch away during arm
        motion.
...
def main(args=None):
    ...
    isaac_sim_gripper_driver = IsaacSimGripperActionServer()

    executor = MultiThreadedExecutor()
    executor.add_node(isaac_sim_gripper_driver)
    try:
        executor.spin()
    except KeyboardInterrupt:
        isaac_sim_gripper_driver.get_logger().info(
            'KeyboardInterrupt, shutting down.\n'
        )
    isaac_sim_gripper_driver.destroy_node()
    rclpy.shutdown()
