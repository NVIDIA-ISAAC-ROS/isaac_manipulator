...
from isaac_manipulator_ros_python_utils import (
    ...
    get_robot_control_nodes, get_robot_state_publisher, get_ur_drivers,
    get_visualization_node, start_tool_communication, UrRobotiqDriverConfig, WorkflowType
)
...
def launch_setup(context, *args, **kwargs):
    ...
    if not driver_config.use_sim_time and (
            driver_config.workflow_type == WorkflowType.POSE_TO_POSE or
            driver_config.workflow_type == WorkflowType.OBJECT_FOLLOWING):
        driver_nodes = get_ur_drivers(driver_config, core_config)
        if core_config.enable_rviz_visualization:
            driver_nodes.append(get_visualization_node(
    ...
    ros2_control_nodes = get_robot_control_nodes(driver_config)
    move_group_node, moveit_config = get_moveit_group_node(driver_config)
    if core_config.enable_rviz_visualization:
        manipulator_init_nodes.append(
