...
def _resolve_policy_class_and_kwargs(agent_cfg):
    # Map old or new agent.yaml formats to a policy class and constructor kwargs.
    policy_cfg = agent_cfg.get('policy', {})
    if 'class_name' in policy_cfg:
        policy_class = eval(policy_cfg.pop('class_name'))
        return policy_class, policy_cfg

    if 'actor' not in agent_cfg:
        raise ValueError(
            'Cannot determine policy class from agent config. '
            "Expected 'policy.class_name' or 'actor' section.")

    actor_cfg = agent_cfg['actor']
    critic_cfg = agent_cfg.get('critic', actor_cfg)
    dist_cfg = actor_cfg.get('distribution_cfg') or {}

    is_heteroscedastic = 'Heteroscedastic' in dist_cfg.get('class_name', '')
    ...
    kwargs = {
        ...
        'rnn_type': actor_cfg.get('rnn_type', 'lstm'),
        'rnn_hidden_dim': actor_cfg.get('rnn_hidden_dim', 256),
        'rnn_num_layers': actor_cfg.get('rnn_num_layers', 1),
        'init_noise_std': dist_cfg.get('init_std', 1.0),
        'noise_std_type': dist_cfg.get('std_type', 'scalar'),
        'state_dependent_std': is_heteroscedastic,
    }
    return ActorCriticRecurrent, kwargs
...
def load_agent_rsl_rl(checkpoint_filepath):
    ...
    policy_class, policy_kwargs = _resolve_policy_class_and_kwargs(agent_cfg)
    ...
    policy = policy_class(obs_dict, obs_groups, num_actions, **policy_kwargs)
    ...
    if 'model_state_dict' in loaded_dict:
        ...
        _KEY_MAP_ACTOR = {'obs_normalizer.': 'actor_obs_normalizer.',
                          'mlp.': 'actor.', 'rnn.': 'memory_a.'}
        actor_weights = {}
        for key, val in loaded_dict['actor_state_dict'].items():
            for prefix, replacement in _KEY_MAP_ACTOR.items():
                if key.startswith(prefix):
                    key = replacement + key[len(prefix):]
                    break
            actor_weights[key] = val
        policy.load_state_dict(actor_weights, strict=False)
    else:
        raise KeyError(
            f'Checkpoint missing expected keys. Found: {list(loaded_dict.keys())}'
        )
