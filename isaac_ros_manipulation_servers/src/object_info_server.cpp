...
#include "isaac_ros_manipulation_servers/object_info_server.hpp"
  ...
  const std::shared_ptr<rclcpp_action::ServerGoalHandle<GetObjectsAction>> goal_handle)
{
  ...
  std::vector<isaac_ros_manipulation_interfaces::msg::ObjectInfo> objects;
      ...
      backend == ObjectDetectionBackend::RT_DETR)
    {
      ...
      if (maybe_trigger_result) {
        ...
        for (uint32_t i = 0; i < detections.size(); i++) {
          objects.push_back(isaac_ros_manipulation_interfaces::msg::ObjectInfo());
