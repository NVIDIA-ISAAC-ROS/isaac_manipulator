...
class ReadGraspPoses(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        try:
            ...
            # Publish object frame TF for ESDF clearing while planning.
            object_frame_name = object_info.get('object_frame_name')
            if object_frame_name:
                obj_tf = TransformStamped()
                obj_tf.header.stamp = self.node.get_clock().now().to_msg()
                obj_tf.header.frame_id = 'base_link'
                obj_tf.child_frame_id = object_frame_name
                obj_tf.transform.translation.x = position[0]
                obj_tf.transform.translation.y = position[1]
                obj_tf.transform.translation.z = position[2]
                obj_tf.transform.rotation.x = orientation[0]
                obj_tf.transform.rotation.y = orientation[1]
                obj_tf.transform.rotation.z = orientation[2]
                obj_tf.transform.rotation.w = orientation[3]
                self.tf_broadcaster.sendTransform(obj_tf)
                self.node.get_logger().info(
                    f"[{self.name}] Published object frame TF '{object_frame_name}' "
                    f'for object_id={self.blackboard.active_obj_id}')
