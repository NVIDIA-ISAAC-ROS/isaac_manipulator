...
class IsaacRosManipulationGearAssemblyOrchestrator(RCLPYNode):
    """
    ...
    def publish_pose_on_tf_static(self, grasp_pose: Pose, parent_frame: str, child_frame: str):
        ...
        # Only in sim: remove any existing transform for this child_frame so that
        # sendTransform() replaces it rather than silently ignoring the update.
        # The Jazzy StaticTransformBroadcaster only appends frames that are not
        # already in _child_frame_ids, so we must clear the old entry first.
        # Skipped on real robot to avoid touching broadcaster internals.
        if self._use_sim_time:
            self.tf_static_broadcaster._child_frame_ids.discard(child_frame)
            self.tf_static_broadcaster.net_message.transforms = [
                t for t in self.tf_static_broadcaster.net_message.transforms
                if t.child_frame_id != child_frame
            ]
    ...
    def _get_current_joint_state(self, timeout_sec: float = 2.0) -> JointState | None:
        """Fetch a single joint state message on demand."""
        success, msg = wait_for_message(JointState, self, '/isaac_joint_states',
                                        time_to_wait=timeout_sec)
        if not success:
            self.get_logger().warn('Timed out waiting for /isaac_joint_states')
            return None
        return msg

    def _publish_joint_state_cmd(self, state: JointState):
        """Publish a fixed joint state as a command to /isaac_joint_commands."""
        if state is None:
            self.get_logger().warn('No joint state to publish')
            return
        cmd = JointState()
        cmd.header.stamp = self.get_clock().now().to_msg()
        cmd.name = state.name
        cmd.position = list(state.position)[:6]
        cmd.velocity = [0.0] * len(cmd.position)
        cmd.effort = [0.0] * len(cmd.position)
        self._sim_joint_commands_publisher.publish(cmd)
    ...
    def gear_assembly(self) -> bool:
        ...
        for idx, gear_type in enumerate(gears_to_insert):
            ...
            if self._run_rl_inference:
                ...
                if self._use_sim_time:
                    # rl_insertion() returns only after the action server result is
                    # received. The action server's finally block sends CANCEL_INSERTION
                    # before returning, which stops the goal_pose_publisher and the
                    # entire RL pipeline. Snapshot the joint state now and repeatedly
                    # publish that fixed value so the position controller is primed with
                    # exactly the post-insertion pose when we switch back to it.
                    hold_state = self._get_current_joint_state()
                    hold_timer = self.create_timer(
                        0.05, lambda: self._publish_joint_state_cmd(hold_state))
                    self._open_gripper()
                    ...
                    # Stop the hold timer now that the position controller is active.
                    hold_timer.cancel()
            ...
            # Now clear objects — only needed when the perception pipeline is active.
            # With ground-truth pose estimation the object tracker never runs,
...
def main(args=None):
    ...
    gear_assembly_orchestrator = IsaacRosManipulationGearAssemblyOrchestrator()
